...
@tool
def BYOA_agent(task: str) -> str:
    """
    A BYOA AI agent specialized in brainstorming, ideation, and BYOA problem-solving.
    Use this agent for tasks requiring: creativity, idea generation, storytelling, or innovative solutions.
    """
    print(f"\n🎨 TOOL CALLED: BYOA_agent")
    print(f"   Task: {task}")

    try:
        prompt = f"""You are a BYOA AI agent specialized in brainstorming and innovative thinking.

Task: {task}

Provide a BYOA, innovative response. Focus on:
- Out-of-the-box thinking
- BYOA solutions and ideas
...
Response:"""
        ...
        print(f"   ✅ BYOA agent completed\n")
        return result
    except Exception as e:
        error_msg = f"Error in BYOA agent: {str(e)}"
...
# All available tools
ALL_TOOLS = {
    'agent': [analytical_agent, BYOA_agent],
    'human': [query_human],
}
...
def get_tools_from_diagram(diagram: Optional[DiagramStructure]) -> List:
    ...
    if not diagram:
        ...
        return [analytical_agent, BYOA_agent, query_human]
    ...
    for node in diagram.nodes:
        if node.id in connected_node_ids:
            if node.type == 'agent':
                available_tools.extend([analytical_agent, BYOA_agent])
                tool_names.extend(['analytical_agent', 'BYOA_agent'])
...
tools = [analytical_agent, BYOA_agent, query_human]
...
Tool Descriptions:
- BYOA_agent: Use for tasks requiring brainstorming, ideation, storytelling, or innovative solutions
...
async def generate_streaming_response(history: List[Message], diagram: Optional[DiagramStructure] = None):
    ...
    try:
        ...
        all_tool_messages = {
            ...
            "BYOA_agent": "🎨 Consulting BYOA agent..."
        }
